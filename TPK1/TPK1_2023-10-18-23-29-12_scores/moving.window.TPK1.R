...
setwd("~/Desktop/Github Projects/Tileseq_Scores/TPK1/TPK1_2023-10-18-23-29-12_scores")

# old score
map.2019 <- read.csv(file = "urn_mavedb_00000001-d-1_scores.csv") %>% 
...
# new score
map.2023 <- read.csv(file = "select_t1_simple.csv", 
                     skip = 16) %>% 
  mutate(mut=substr(hgvs_pro, nchar(hgvs_pro)-2, nchar(hgvs_pro))) %>%
  mutate(wt=substr(hgvs_pro, 3, 5)) %>% 
  mutate(type= ifelse(grepl("=", mut), "synonymous", ifelse(grepl("Ter$", mut), 
...
VARITY_R.score <- read.csv(file = "TPK1[Q9H3S4]_1-243_VARITY_R_20231109033931447811.csv") %>% 
...
VARITY_ER.score <- read.csv(file = "TPK1[Q9H3S4]_1-243_VARITY_ER_20231109034733959751.csv") %>% 
...
ggplot(window.df, aes(x = position)) +
  ...
  theme_prism() +
  labs(title = "Correlation between Fitness Scores & VARITY_R Prob of pathogenicity of TPK1",
       x = "position", y = "Spearman's rho", colour = "Scores") +
  theme(legend.position = c(0.9, 0.9), 
        legend.background = element_rect(fill = "white", colour = "grey22"))
ggsave(filename = "spearman_VARITY_R.png", dpi = 700,
       height = 6, width = 11)
...
ggplot(window.df, aes(x = position)) +
  ...
  scale_color_manual(values = V_colors) +
  theme_prism() +
  labs(title = "Correlation between Fitness Scores & VARITY_ER Prob of pathogenicity of TPK1",
       x = "position", y = "Spearman's rho", colour = "Scores") +
  theme(legend.position = c(0.9, 0.9), 
        legend.background = element_rect(fill = "white", colour = "grey22"))
ggsave(filename = "spearman_VARITY_ER.png", dpi = 700,
       height = 6, width = 11)
...
# ggsave(filename = "output/scores_VARITY_year.png", dpi = 700,
#        height = 6, width = 8)

ggplot(window.df, aes(x = position, y = rho)) +
  geom_line(linewidth = 1) +
  labs(title = "Spearman's Rho between 2023 & 2019 Fitness Score of TPK1",
       x = "position", y = "Spearman's rho") +
  theme_prism()
# ggsave(filename = "score_rho.png", dpi = 700,
#        height = 6, width = 9)