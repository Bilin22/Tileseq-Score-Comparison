...
setwd("/Users/admin/Desktop/Github Projects/Tileseq_Scores/CALM1/Score_comparison")

# old data
map_2019 <- read.csv(file = "data/urn_mavedb_00000001-c-1_scores.csv") %>% 
...
# new data
map_2023 <- read.csv(file = "data/select_t1_simple_aa.csv", 
                     skip = 16) %>% 
  mutate(mut=substr(hgvs_pro, nchar(hgvs_pro)-2, nchar(hgvs_pro))) %>%
  mutate(wt=substr(hgvs_pro, 3, 5)) %>% 
  mutate(type= ifelse(grepl("=", mut), "synonymous", ifelse(grepl("Ter$", mut), 
...
# ggsave(file = "output/se_boxplot.png",
...
# ggsave(file = "output/comparison_mutation.png",
...
overall_mutation <- ggplot(match_df, aes(x = score.2023, y = score.2019)) + 
  ...
  theme(legend.key.size = unit(0.5, "cm")) +
  scale_color_manual(values = c("grey59", "red2", "chartreuse3"))
# ggsave(file = "output/comparison_overall.png",
