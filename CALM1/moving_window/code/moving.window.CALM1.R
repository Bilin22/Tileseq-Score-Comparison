...
setwd("~/Desktop/Github Projects/Tileseq_Scores/CALM1/moving_window/")

# old score
map.2019 <- read.csv(file = "data/urn_mavedb_00000001-c-1_scores.csv") %>% 
...
VARITY_R.score <- read.csv(file = "data/CALM1[P0DP23]_1-149_VARITY_R_20230810204537781836.csv") %>% 
...
VARITY_ER.score <- read.csv(file = "data/CALM1[P0DP23]_1-149_VARITY_ER_20230810205143476932.csv") %>% 
...
ggplot(window.df, aes(x = position)) +
  ...
  theme_bw() +
  labs(title = "Correlation between Fitness Scores & VARITY_R Prob of pathogenicity of CALM1",
...
ggplot(window.df, aes(x = position)) +
  ...
  scale_color_manual(values = V_colors) +
  theme_bw() +
  labs(title = "Correlation between Fitness Scores & VARITY_ER Prob of pathogenicity of CALM1",
       x = "position", y = "Spearman's rho", colour = "Scores") +
  theme(legend.position = c(0.9, 0.9), 
        legend.background = element_rect(fill = "white", colour = "grey22"))
...
ggplot(window.df, aes(x = position, y = rho)) +
  ...
  labs(title = "Spearman Correlation Coefficient between 2023 & 2019 Fitness Score of CALM1",
       x = "position", y = "Spearman's rho") +
  theme_bw()
# ggsave(filename = "output/score_rho.png", dpi = 700,
#        height = 6, width = 8)