...
args <- list(map="f12AV_t1_simple_aa_MAVE.csv",
             reference="MTHFR_refVars.csv",
             predictors="f25AV_t1_simple_aa_MAVE.csv,f100AV_t1_simple_aa_MAVE.csv,f200AV_t1_simple_aa_MAVE.csv",
predictorNames= "f25AV,f100AV,f200AV",
predictorOrders="d,d,d",outfile="testPRC",
posRanges="2-147, 148-294, 295-474, 475-656",
logfile="prc.log",
noBalancing = FALSE,
noMono = FALSE,
labelScores = FALSE)
...
# posRanges <- c(posRanges,list(c(2,maxPos)))
...
lapply(posRanges, function(range) {
  ...
  # refsubset <- refset[which(refset$pos >= range[[3]] & refset$pos <= range[[4]]),]
